    ...
    <Dialog onOpenChange={nextOpen => { if (!nextOpen) onClose(); }} open={open}>
      <DialogContent className="max-w-sm" showCloseButton={false}>
        <DialogHeader>
          ...
          <DialogDescription>{t.avatarEditor.description}</DialogDescription>
        </DialogHeader>
        <button
        ...
        <div className="space-y-4 py-2">
          <ParticipantRow
            label={t.avatarEditor.youLabel}
            onNameChange={name => handleNameChange('user', name)}
            placeholder={t.avatarEditor.placeholder}
            role="user"
            savedName={names.user || DEFAULT_NAMES.user}
          />
          <ParticipantRow
            label={t.avatarEditor.hermesLabel}
